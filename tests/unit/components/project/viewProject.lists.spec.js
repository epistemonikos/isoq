...
describe('viewProject.vue — createList()', () => {
  ...
  it('does not send a separate last_update PATCH (the backend seals it on write)', async () => {
    Api.post.mockResolvedValueOnce({ data: { id: 'list1', name: 'F' } })
    const { wrapper } = createWrapper()
    await wrapper.setData({ summarized_review: 'F' })
    jest.spyOn(wrapper.vm, 'createFinding').mockResolvedValue()
    wrapper.vm.createList()
    await flushPromises()
    const lastUpdatePatch = Api.patch.mock.calls.find(
      c => typeof c[0] === 'string' && c[0].startsWith('/isoqf_projects/') &&
        c[1] && Object.prototype.hasOwnProperty.call(c[1], 'last_update')
    )

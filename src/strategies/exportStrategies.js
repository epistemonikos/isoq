import { Document, Packer, Paragraph, TextRun, HeadingLevel, AlignmentType, Table, TableCell, TableRow, WidthType, VerticalAlign, PageOrientation } from 'docx'
...
const getFormattedExplanation = (type, option, explanation) => {
  return displayExplanation(type, option, explanation) || explanation || ''
}
...
export class WorksheetExportStrategy extends BaseExportStrategy {
  constructor (project, data, locale = 'en') {
    super(project, data, locale)
    this.levelConfidenceOptions = [
  ...
  async export () {
    ...
    this.addHeader(ep)
    this.createEvidenceProfileTable(ep)

    if (isPublic) {
      this.addOptionalTable('Characteristics of Studies', this.data.characteristicsStudies)
      this.addOptionalTable('Methodological Assessments', this.data.methodologicalAssessments)
      this.addOptionalTable('Extracted Data', this.data.extractedData)
    }
  ...
  addHeader (evidenceProfile) {
    this.builder.addParagraph((this.project && this.project.name) || '', {
      size: 24,
      bold: true,
      heading: HeadingLevel.HEADING_2
    })

    this.builder.addParagraph('GRADE-CERQual Assessment Worksheet', {
      size: 28,
      bold: true,
      heading: HeadingLevel.HEADING_1,
      alignment: AlignmentType.CENTER
  ...
  createEvidenceProfileTable (evidenceProfile) {
    ...
    if (evidenceProfile && (evidenceProfile.name || evidenceProfile.methodological_limitations || evidenceProfile.coherence)) {
      ...
      const formatCell = (type, option, explanation) => {
        ...
        if (!displayOpt && !formattedExp) return ''
        if (displayOpt && !formattedExp) return displayOpt
        if (displayOpt && formattedExp) return `${displayOpt}\n\nExplanation: ${formattedExp}`
        return `Explanation: ${formattedExp}`
      }
      ...
      const dataRow = [
        ...
        { text: formatCell('adequacy', evidenceProfile.adequacy?.option, evidenceProfile.adequacy?.explanation), verticalAlign: VerticalAlign.TOP },
        { text: formatCell('relevance', evidenceProfile.relevance?.option, evidenceProfile.relevance?.explanation), verticalAlign: VerticalAlign.TOP },
        { text: formatCell('cerqual', evidenceProfile.cerqual?.option, evidenceProfile.cerqual?.explanation), verticalAlign: VerticalAlign.TOP },
  ...
  addOptionalTable (title, data) {
    if (!data || !data.items || !Array.isArray(data.items) || data.items.length === 0) {
      return
    }

    this.builder.addSpacing()
    this.builder.addParagraph(title, { size: 24, bold: true })
    this.addGenericTable(data)
  }

  addGenericTable (data) {
    ...
    for (const item of (data.items || [])) {
      ...
      for (const field of data.fields) {
        if (field.key !== 'ref_id' && field.key !== 'actions') {
          ...
          row.push({ text: this.safeString(value), verticalAlign: VerticalAlign.TOP })
        }
      }
      if (row.length > 0) {
        rows.push(row)
  ...
  displayOption (option) {
    if (option === null || option === undefined) return ''
    const numOption = typeof option === 'string' ? parseInt(option, 10) : option
    const opt = this.selectOptions.find(o => o.value === numOption)
    return opt ? opt.text : ''
  }
  ...
  displayLevelConfidence (option) {
    if (option === null || option === undefined) return ''
    const numOption = typeof option === 'string' ? parseInt(option, 10) : option
    const opt = this.levelConfidenceOptions.find(o => o.value === numOption)
    return opt ? opt.text : ''
  }
...
export class ExportStrategyFactory {
  static createStrategy (type, project, data, locale = 'en') {
    switch (type) {
      ...
      case 'worksheet':
        return new WorksheetExportStrategy(project, data, locale)
      default:

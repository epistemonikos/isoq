<template>
  <b-modal
    ...
    @hidden="reset"
  >
    <p v-if="user">{{ $t('admin.deactivate_confirm', { name: fullName }) }}</p>
...
export default {
  ...
  methods: {
    ...
    async confirm () {
      ...
      this.error = ''
      try {
        await Api.patch(`/admin/users/${this.user.id}/deactivate`)
        this.$emit('deactivated', this.user.id)

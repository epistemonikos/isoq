...
export default {
  ...
  methods: {
    ...
    confirmExportData: async function () {
      ...
      try {
        await downloadPersonalData(this.$store.state.user.id, this.exportPassword)
        this.$bvModal.hide('modal-export-data')
      } catch (error) {
        // El servicio deja en backendMessage el texto del servidor, o null
        // cuando la respuesta no era un JSON con message.
        this.exportError = error.backendMessage || this.$t('gdpr.export.genericError')
      } finally {

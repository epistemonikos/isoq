<template>
  <div>
          ...
          <template v-else>
            ...
            <b-td
              ...
              <template v-if="isPublic && project.public_type !== 'minimally'">
                <p>
                  <b-link :to="{ name: 'previewWorksheet', params: { id: item.id, projectId: project.id, token: token } }">{{ item.name }}</b-link>
                </p>
              </template>
              <template v-else-if="hasPermission">
                <template v-if="item.ref_list && item.ref_list.length">
                  <p>
                  {{ item.name }}
                  </p>
                </template>
    ...
    <template v-if="!onlySummary && (!isPublic || (isPublic && project.public_type !== 'minimally'))">
      <backToTop></backToTop>
      <div class="mt-5">

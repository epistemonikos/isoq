<template>
  <b-modal id="modal-remove-project" ref="modal-remove-project" :title="$t('common.delete_project')" @ok="removeProject"
    ...
    <div v-if="isShared" class="mt-3">
      ...
      <b-form-group :label="$t('common.select_new_owner')">
        <b-form-select v-model="newOwner" :options="eligibleUsersOptions" required>
          <template #first>
            <b-form-select-option :value="null" disabled>-- {{ $t('common.select_new_owner') }}
    ...
    <b-form-group :label="$t('common.enter_password_confirm')" class="mt-3">
      <b-form-input type="password" v-model="password" required :placeholder="$t('common.password')">
      </b-form-input>
    </b-form-group>

    <b-alert variant="danger" show v-if="serverError" dismissible @dismissed="serverError = null" class="mt-3">
      {{ serverError }}
    </b-alert>
  </b-modal>
</template>
